...
!!! note
    ...
    I used to automatically copy Jupyter Notebooks from `./notebooks` into the documentation, but this created two
    problems. First, a LOT of the notebooks were copied, which were not needed in the documentation. Second, frequently
    I did something to alter the Notebook I did not really want in the documentation.

    Hence, to avoid these two issues, now the template requires deliberately moving the Jupyter Notebooks you want to
    include in the documentation from `./notebooks` to `./docsrc/mkdocs/notebooks`.
...
you create a script in the scripts directory, you can configure the root logger as needed for that
...
if __name__ == "__main__":
    ...
    # omitting the name uses the root logger - will output both to console and logfile
